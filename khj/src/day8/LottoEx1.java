...
public class LottoEx1 {
	...
	/* 기능 : 배열과 정수가 주어지면 정수가 배열에 있으면 true를, 없으면 false를 알려주는 메소드
	 * 매개변수 : 주어진 정수 = > int num1 , 주어진 배열 = > int lotto[]
	 * 리턴타입 : ture/false => boolean
	 * 메소드명 : containsArray
	 * */
	...
	public static int[] randomArray(int min, int max, int size) {
		...
		/* 기능: 최소값, 최대값, 배열이 주어지면 min~max사이의 랜덤한 수를 생성하여 배열에 있는지 없는지 확인한 후 없으면 해당 수를 알려주는 메소드
	...
	public static int createBonus(int min, int max, int []arr) {
		...
		while(true) {
			...
			break; // 당첨번호에 보너스 번호가 없으면 반복문 종료
		}
		return bonus;
	}
	//등수를 출력
	...
	public static int check(int arr1[], int arr2[]) {
		 ...
		 * 매개변수 : 로또 번호와 보너스 번호, 사용자 번호= > int lotto[], int bonus, int user[]
		 * 리턴타입 : 없음 => void
		 * 메소드명 : rank
		 * */
		...
		public static void rank(int lotto[], int bonus, int user[]) {
			...
			case 5 : result = containsArray(user, bonus, user.length) ? "2등 당첨!!" : "3등 당첨";break; 
