...
public class ExeBaseballEx1 {
	...
	public static void main(String[] args) {
		...
		// 반복 : 다 맞출때 까지 반복 = > 3스트라이크가 될때 까지
		while(strike != count ) {
			System.out.println("입력(예 : 1 2 3) : ");
	...
	/* 기능 : 정수 리스트가 주어지면 해당 정수 리스트에 min~max 사이의 값을 중복되지 않게
	 * 			count개를  저장하는 메소드
	 * 매개변수 : 주어진 정수 리스트, 정수 범위, 저장한 갯수 => 
	 ...
	 * 		 count개 입력받아 제대로 입력 했는지를 알려주는 메소드
	 * 		 0 : 제대로 입력, 1 : 범위 잘못, -1 : 중복
	 * 매개변수 : 리스트, 범위, 갯수, 스캐너 = > List<Integer> list, int min, int max, 
	...
	public static int inputList(List<Integer>list, int min, int max, int count, Scanner scan) {
		...
		if(scan == null) {
			throw new NullPointerException("스캐너가 null입니다.");
		}
	...
	/* 기능: 두 정수 리스트가 주어지면 다른 번지에 있는 같은 숫자의 개수를 알려주는 메소드
	 * 매개변수 : 두 정수 리스트 => List<Integer>list1, List<Integer>list2
	 * 리턴타입 : 같은 번지에 있는 숫자 중 같은 숫자의 개수 => int
	 * 메소드명 : getBall*/
