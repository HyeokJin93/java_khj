...
- 장점
  ...
  2. 처리시간이 줄어든다
  3. 응용 프로그램측 로직을 거치지 않고도 데이터의 앞뒤가 맞게 할 수 있다
  4. 유지보수에 뛰어나다
- 단점
  1. 재사용성이 나쁘다
...
-- 과일 판매시 판매 금액을 자동으로 입력하여 판매 정보를 sell테이블에 추가하는 프로시저
...
begin
    ...
    set _total_price = _price * in_amount;
		insert into sell
			select max(se_num)+1, in_fr_name, in_amount, in_unit, _totla_price, now(), in_type
				from sell;
end //
delimiter ;
...
/*
insert트리거에서 새로 추가된 행에 update를 못함. 이럴 땐 트리거를 이용해야함
트리거는 이벤트마다 동일하게 처리하는 경우 사용, 아니면 프로시저 사용
통신사에서 작년 사용 금액을 기준으로 올해 등급을 결정하는 과정을 하려 한다.
이때 프로시저? 트리거? => 프로시저
제품을 구매할 때 마다 구매 금액의 10프로를 포인트로 지급하려 한다.
이때 프로시저? 트리거? => 트리거
*/