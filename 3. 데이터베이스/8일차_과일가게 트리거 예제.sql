...
- 데이터 무결성을 지킬 수 있다 => 연결된 테이블간의 데이터 일관성을 유지
- 트리거를 사용방법
  1. 트리거 생성
  2. 이벤트를 발생
- 트리거 기본 형태
delimiter //
create trigger 트리거명 타이밍(after|befroe) 이벤트명(insert|update|delete) on 테이블명
for each row
begin
-- 실행코드
end//
delimiter ;
  - old : update 이벤트 발생할 때 사용 가능한 것으로, 값이 바뀌기 전의 행
  - new : insert 또는 update 이벤트 발생할 때 사용한 것으로, 값이 바뀐후의 행 또는 새로 추가된 행
  - 변수 선언 방법
...
-- 과일을 판매했을 때 수량을 맞춰주는 트리거를 만들어보세요.
delimiter //
...
begin
	...
	-- 판매한 과일의 수량 : new.se_amount
	-- 판매한 과일의 이름 : new.se_fr_name
	update fruit
		set
...
insert into sell values (2, '바나나', 30, '송이', 4000, now(), '현금');
