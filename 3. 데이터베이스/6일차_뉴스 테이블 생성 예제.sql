/*
스포츠, 연예, 일반 뉴스를 관리하는 사이트를 구축하기위한 erd를 설계하세요
- 네이버 뉴스 참고
- 뉴스 등록, 수정, 삭제 기능
- 댓글 기능
- 좋아요, 싫어요 기능
...
- 뉴스
  - 뉴스번호(pk), 제목, 내용, 기자번호(fk), 작성일, 최종수정일, 신문사, 대분류(일반/스포츠/연예), 
	중분류(경제, 정치, 야구, 축구 등)
- 기자
  - 기자번호(pk), 이름, 주민번호, 구독자수, 응원수
- 회원
  - 아이디(pk), 비번
- 댓글
  - 댓글번호(pk), 뉴스번호(fk), 아이디(fk), 댓글내용, 공감, 비공감, 답글번호(fk), 작성일
- 반응
  - 반응번호(pk), 뉴스번호(fk), 아이디(fk), 상태(좋아요, 훈훈해요, 슬퍼요)
*/

CREATE DATABASE news_khj;
USE news_khj;
...
CREATE TABLE `news` (
	...
	`nw_datetime`	datetime	NULL,
	`nw_upudate`	datetime	NULL,
	`nw_company`	varchar(50)	NULL,
	`nw_mian_category`	varchar(50)	NULL,
