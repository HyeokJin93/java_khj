-- 1. university_khj DB 생성, 있으면 삭제 후 생성
DROP SCHEMA IF EXISTS `university_khj`;
CREATE SCHEMA `university_khj` ;
-- 2. university_khj DB 선택
USE university_khj;

-- 3. student 테이블 생성
-- 비밀번호를 255자로 하는 이유 : 일반적으로 비밀번호는 노출되면 안되기 때문에 암호화가 이루어짐 
-- 암호화가 이루어지면 짧은 비밀번호가 긴 문자열로 바뀜 
DROP TABLE IF EXISTS `university_khj`.`student`;
CREATE TABLE `university_khj`.`student` (
  ...
  -- 테이블명 앞에 DB명이 붙는 경우와 안붙는 경우의 차이 
  -- 테이블명 앞에 DB명이 붙으면 현재 선택된 DB와 상관없이 테이블을 활용할 수 있다
  -- 테이블명 앞에 DB명이 안붙으면 현재 선택된 DB의 테이블을 활용할 수 있다
  -- world DB가 선택된 상태에서 create table student();를 하면 world DB안에 student 테이블을 생성
  -- world DB가 선택된 상태에서 create table university_khj.student();를 하면
  -- 선택된 DB와 상관없이 university_khj DB안에 student 테이블을 생성
  
  
  -- 4. professor 테이블 생성
  DROP TABLE IF EXISTS `university_khj`.`professor`;
  CREATE TABLE `university_khj`.`professor` (
...
-- 5. subject 테이블 생성
DROP TABLE IF EXISTS `university_khj`.`subject`;
CREATE TABLE `university_khj`.`subject` (
  `su_num` INT NOT NULL AUTO_INCREMENT,
  `su_code` CHAR(6) NOT NULL,
  `su_schedule` VARCHAR(30),
  `su_class_num` INT DEFAULT '1',
  `su_title` VARCHAR(30),
  PRIMARY KEY (`su_num`),
  UNIQUE INDEX `su_code_UNIQUE` (`su_code` ASC) VISIBLE);
  
  
  -- 6. cousre 테이블 생성
  DROP TABLE IF EXISTS `university_khj`.`course`;
  CREATE TABLE `university_khj`.`course` (
  ...
  CONSTRAINT `co_st_num` FOREIGN KEY (`co_st_num`)
    REFERENCES `university_khj`.`student` (`st_num`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `co_su_num` FOREIGN KEY (`co_su_num`)
    REFERENCES `university_khj`.`subject` (`su_num`) ON DELETE CASCADE ON UPDATE CASCADE);
    
-- 7. lecture 테이블 생성
DROP TABLE IF EXISTS `university_khj`.`lecture`;
CREATE TABLE `university_khj`.`lecture` (
  ...
  CONSTRAINT `le_pr_num`FOREIGN KEY (`le_pr_num`)
    REFERENCES `university_khj`.`professor` (`pr_num`) ON DELETE CASCADE ON UPDATE CASCADE,
