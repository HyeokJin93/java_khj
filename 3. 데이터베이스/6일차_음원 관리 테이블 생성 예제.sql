...
- 추천
  - 한명의 회원은 음원 하나 당 한번만 추천이 가능하고, 한번 더 누르면 취소 
  - 하나의 음원에는 여러 회원이 추천 가능
- 재생목록
  - 회원 당 여러 재생 목록을 가질 수 있다.
...
- 구매
  ...
  - 모든 음원은 고정되고 동일한 가격이다. 할인x, 이벤트x
  - 한달 이용권 x
  - 구매한 음원은 취소 불가능
- 회원 등급x
*/
CREATE DATABASE melon_khj;
USE meolon_khj;
...
CREATE TABLE `act` (
	`ac_num`	int	NOT NULL,
	`ac_at_num`	varchar(50)	NOT NULL,
	`ac_gr_num`	int	NOT NULL,
	`ac_roll`	varchar(10)	NULL
);
