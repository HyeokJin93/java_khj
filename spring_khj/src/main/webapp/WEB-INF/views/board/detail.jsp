...
<body>
			...
			$.ajax({
				...
				success : function(res){
					...
					if(res == true){
						...
						// 새로고침(전체가 아닌 댓글부분만)을 해서 1페이지에 맞는 댓글을 새로 가져옴
						var co_bd_num = '${board.bd_num}'; 
	...
	// 요소에 이벤트를 등록하는게 아니라 document에 등록해서 요소가 나중에 추가돼도
	// 해당 선택자만 맞으면 이벤트가 실행되도록 함
	...
	// 댓글 삭제
		...
		if(co_num != ''){
			$.ajax({
				async:false,
				// 삭제할 댓글 번호는 url로 노출되어도 큰 문제가 없으므로 get으로 보냄
				type:'get',
	...
	$(document).on('click','.comment-list .btn-mod-comment', function(){
		// 이전에 생긴 (댓글 수정 버튼을 클릭해서 생긴)textarea태그와 등록 버튼을 제거하고,
		// 감춘 답글, 수정, 삭제 버튼을 추가
		...
		// 등록된(수정 전)댓글 내용을 가져옴. textarea태그 안에 넣어주기 위해
		...
		var btnStr =
			'<button class="btn-mod-comment2 btn btn-outline-warning ml-2">등록</button>';
		//기존 댓글을 감춤
	...
	// 게시글의 댓글 중 page 번호에 맞는 댓글을 가져와서 화면에 배치하는 함수
			...
			$.ajax({
				...
				success : function(res){
				    ...
				    for(tmp of res.list){
				    	...
				    	// 댓글 정보를 html로 만들어진 문자열로 만든 후 이어 붙임
				    	str +=
	...
	function createCommentPagination(pm){
		...
		str +=
		'<ul class="pagination justify-content-center">';
		var startDisabled = pm.start ? '' : 'disabled';
		var endDisabled = pm.end ? '' : 'disabled';
